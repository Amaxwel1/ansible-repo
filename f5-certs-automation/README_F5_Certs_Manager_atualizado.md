...
Playbook para **inventário, rotação, validação e limpeza de certificados no F5 BIG-IP**, com suporte a execução em **modo list/check/apply**, descoberta automática de **certificado atual**, **Client SSL Profiles** impactados, **Virtual Servers / VIPs** relacionados, validação **TLS + HTTP**, **Config Sync HA** e relatório HTML consolidado.
...
- executa validação TLS + HTTP ao final do grupo;
...
| `f5_duplicate_cleanup_only_if_not_in_use` | bool | Se `true`, não remove duplicado ainda referenciado por profile/chain. Se `false`, pode reapontar o profile para outro certificado válido antes de remover o antigo. |
...
| `f5_enable_chain_on_profile` | bool | Se `true`, associa a chain importada ao Client SSL Profile. |
...
- **Relatório consolidado**: o HTML final inclui resumo do `apply`, inventário por site, duplicidade, cleanups, Config Sync e resultado das validações.
...
### 3.3 Exemplo com sites internos/externos
...
### 3.5 Exemplo de item interno montado automaticamente
...
| `f5_certs_manager/defaults/main.yml` | Parâmetros padrão da role: credenciais, sites, CIFS, thresholds, cleanup, cadeia, Config Sync e relatório. |
...
| `41_apply_one_rotation__per_site_profile.yml` | Importa cert/key/chain e atualiza o Client SSL Profile no F5. |
...
### 6.7 Descoberta dos targets de validação
...
- se `f5_duplicate_cleanup_only_if_not_in_use=false`, a automação pode reapontar o profile para outro certificado válido antes de remover o antigo.
...
- não estão em uso por profile;
- não estão em uso como chain;
